...
export function SimpleNavigation() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-lg border-b border-slate-200 transition-all duration-300">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <div className="flex items-center space-x-4">
              <FagriLogo className="h-20 w-20" />
              <span className="text-4xl font-light text-slate-800 transition-colors duration-300">
                FAGRI.Digital
              </span>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navigationItems.map((item) => (
                <button
                  ...
                  className="text-sm font-light text-slate-700 hover:text-emerald-600 transition-colors duration-300"
                >
                  {item.label}
                </button>
              ))}
              ...
              <button
                ...
                className="px-3 py-1 text-sm font-medium rounded-full border text-slate-700 border-slate-300 hover:bg-slate-100 transition-all duration-300"
              >
                {language === 'it' ? 'EN' : 'IT'}
              </button>
            ...
            <div className="md:hidden flex items-center space-x-3">
              ...
              <button
                onClick={handleLanguageToggle}
                className="px-2 py-1 text-xs font-medium rounded border text-slate-700 border-slate-300 transition-all duration-300"
              >
                {language === 'it' ? 'EN' : 'IT'}
              </button>
